CREATE TABLE IF NOT EXISTS supervisor_rutas_semanales (
  ...
  UNIQUE KEY uk_supervisor_ruta_semana (semana_inicio, sede, ruta),
...
ALTER TABLE supervisor_rutas_semanales
  ADD COLUMN placa_reportada VARCHAR(30) NULL AFTER unidad_id,
  ADD COLUMN identificador VARCHAR(80) NOT NULL DEFAULT '' AFTER placa_reportada;

ALTER TABLE supervisor_rutas_semanales
  DROP INDEX uk_supervisor_ruta_semana,
  ADD UNIQUE KEY uk_supervisor_ruta_unidad_semana (semana_inicio, sede, ruta, identificador);

ALTER TABLE supervisor_rutas_movimientos
  ADD COLUMN placa_reportada_anterior VARCHAR(30) NULL AFTER unidad_id_anterior;

ALTER TABLE supervisor_rutas_movimientos
  ADD COLUMN placa_reportada_nueva VARCHAR(30) NULL AFTER unidad_id_nueva;
