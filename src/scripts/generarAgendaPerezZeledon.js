...
/* ================== SCRIPT ================== */
async function generarAgendaPerezZeledon() {
  try {
    console.log("⏳ Generando agenda PÉREZ ZELEDÓN...");

    // 🔹 Unidades SOLO de Pérez Zeledón
    const [unidades] = await pool.query(`
      SELECT id, placa
      FROM unidades
      WHERE sede = 'Perez Zeledon'
      ORDER BY placa
    `);
    ...
    for (const unidad of unidades) {
      ...
      await pool.query(`
        ...
        VALUES
          (?, 'PREVENTIVO', 'PROGRAMADO', 'MEDIA', ?, 'Perez Zeledon')
      `, [
        unidad.id,
