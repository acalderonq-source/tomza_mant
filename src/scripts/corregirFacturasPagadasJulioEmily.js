...
const ARCHIVO_EMILY = "PAGOS JULIO EMILY.xlsx";
const OBS_AJUSTE = "Ajuste cierre facturas Emily julio 2026";
const ARCHIVOS_PAGOS_PROVEEDOR_JULIO = [
  "PAGO DE PROVEDOR 17-07-2026 QUIROS.xlsx",
  "PLANTILLA PAGOS 20-07-2026.xlsx",
  "PLANTILLA PAGOS 02-07-2026.xlsx",
  "PLANTILLA PAGOS 13-07-2026.xlsx",
  "PAGO DE PROVEDOR 29-07-2026 DaQU.xlsx",
  "PLANTILLA PAGOS 11-07-2026.xlsx",
  "PAGO DE PROVEDOR 07-07-2026 DaniC.xlsx",
  "PLANTILLA PAGOS 14-07-2026.xlsx",
  "PLANTILLA PAGOS 08-07-2026.xlsx",
  "PAGO DE PROVEDOR 15-07-2026 DaniP.xlsx",
  "PLANTILLA PAGOS 25-07-2026.xlsx",
  "PLANTILLA PAGOS 27-07-2026.xlsx",
  "PLANTILLA PAGOS 24-07-2026.xlsx",
  "PAGO DE PROVEDOR 06-07-2026 DaniP.xlsx",
  "PLANTILLA PAGOS 17-07-2026.xlsx",
  "PLANTILLA PAGOS 31-07-2026.xlsx",
  "PAGO DE PROVEDOR 27-07-2026 DaPAM.xlsx",
  "PLANTILLA PAGOS 28-07-2026.xlsx",
  "PLANTILLA PAGOS 23-07-2026.xlsx",
  "PLANTILLA PAGOS 04-07-2026.xlsx",
  "PLANTILLA PAGOS 21-07-2026.xlsx",
  "PLANTILLA PAGOS 27-06-2026.xlsx",
  "PLANTILLA PAGO Llantas Pema S A  Placa C165306 cambio de llantas y rotacion Julio 2026.xlsx",
  "PLANTILLA PAGOS (1).xlsx",
  "PLANTILLA PAGOS 18-07-2026.xlsx",
  "PAGO DE PROVEDOR 08 07 2026 PEMA.xlsx",
  "PLANTILLA PAGOS 30-07-2026.xlsx",
  "PLANTILLA PAGOS 03-07-2026.xlsx",
  "PLANTILLA PAGOS 25-07-2026 IVA.xlsx"
];
...
async function readWorkbook(filePath) {
  ...
  for (const sheet of workbook.worksheets) {
    ...
    sheet.eachRow({ includeEmpty: false }, row => {
      ...
      if (!headerMap) return;
      const normalizedRow = normText(text);
      if (
        normalizedRow.includes("TOTAL SOLICITADO") ||
        normalizedRow.includes("TOTAL GENERAL") ||
        normalizedRow.startsWith("NOTAS:")
      ) return;
      ...
      rows.push({
        ...
        placa: normalizePlate(cellText(row.getCell(headerMap.placa))),
        monto,
...
async function loadFacturas(connection) {
  const [rows] = await connection.query(`
    SELECT id, numero_factura, proveedor_nombre, monto, nota_credito_monto, factura_observacion
    FROM facturas
...
async function main() {
  ...
  const insertProviderCache = new Map();

  try {
    await ensureSchema(connection);
    await connection.beginTransaction();

    await connection.query("DELETE FROM pagos_proveedor WHERE archivo_nombre = ?", [ARCHIVO_EMILY]);
    await connection.query(
      `UPDATE pagos_proveedor
       SET periodo_cierre = ?
       WHERE archivo_nombre IN (?)
         AND COALESCE(pagada, 0) = 1
         AND COALESCE(fecha_pago, fecha_solicitud, DATE(creado_en)) <= '2026-07-31'`,
      [PERIODO_CIERRE, ARCHIVOS_PAGOS_PROVEEDOR_JULIO]
    );
    ...
    for (const row of rows) {
      ...
      if (!insertProviderCache.has(row.proveedor_nombre)) {
        insertProviderCache.set(row.proveedor_nombre, await getProveedorId(connection, row.proveedor_nombre));
      }
      ...
      await connection.query(
        ...
        [
          ...
          row.fecha,
          row.monto,
          insertProviderCache.get(row.proveedor_nombre),
    ...
    const [[resultado]] = await connection.query(`
      SELECT COUNT(*) AS registros,
             ROUND(COALESCE(SUM(
               CASE
                 WHEN COALESCE(monto_pagado_cierre, 0) > 0 THEN monto_pagado_cierre
                 WHEN COALESCE(abono_monto, 0) > 0 THEN LEAST(abono_monto, GREATEST(COALESCE(monto, 0) - COALESCE(nota_credito_monto, 0), 0))
                 ELSE GREATEST(COALESCE(monto, 0) - COALESCE(nota_credito_monto, 0), 0)
               END
             ), 0), 2) AS total
      FROM facturas
      WHERE periodo_cierre = ?
        AND COALESCE(pagada, 0) = 1
    `, [PERIODO_CIERRE]);

    const totalDb = Number(resultado.total || 0);
    if (Math.abs(totalDb - TOTAL_OFICIAL) >= 0.01) {
      throw new Error(`Facturas pagadas quedaria en ${totalDb}, no ${TOTAL_OFICIAL}. Se revierte.`);
    }

    await connection.commit();
    console.log(`Facturas pagadas ${PERIODO_CIERRE}: ${resultado.registros} movimientos, total ${totalDb.toFixed(2)}.`);
    console.log(`Actualizadas: ${actualizadas}. Insertadas: ${insertadas}. Pagos proveedor restaurados.`);
  } catch (error) {
    try {
