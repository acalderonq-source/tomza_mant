...
async function vistaFormulario(req, res, { row, error = '', body, status = 200 }) {
  ...
  const [unidades] = a.secciones[seccion].unidad
    ? await pool.query('SELECT id, placa, marca, modelo, anio, sede FROM unidades ORDER BY placa') : [[]];
  ...
  res.status(status).render('aresep/formulario', {
    a, user: req.session.user, seccion, periodo, row, unidades, historial, error,
...
router.get('/', contexto, envolver(async (req, res) => {
  ...
  const [rows] = await pool.query('SELECT * FROM aresep_registros WHERE periodo = ? AND seccion = ? ORDER BY id DESC', [periodo, seccion]);
  const registros = rows.map(a.preparar);
  ...
  const [cuentas] = await pool.query('SELECT seccion, COUNT(*) AS total FROM aresep_registros WHERE periodo = ? GROUP BY seccion', [periodo]);
  res.render('aresep/index', {
    a, user: req.session.user, periodo, seccion, q, estado, pagina: page, paginas: Math.ceil(visibles.length / 50),
    ...
    cuentas: Object.fromEntries(cuentas.filter(c => a.permitidas(req.session.user).includes(c.seccion)).map(c => [c.seccion, c.total])),
...
async function guardar(req, res, row) {
  ...
  try {
    ...
    if (a.secciones[seccion].unidad) {
      ...
      [[unidad]] = await pool.query('SELECT id, placa FROM unidades WHERE id = ?', [id]);
    }
    ...
    await conn.commit();
    res.redirect(`/aresep?periodo=${periodo}&seccion=${seccion}&guardado=1`);
  } catch (error) {
...
router.post('/incorporar-unidades', contexto, envolver(async (req, res) => {
  ...
  try {
    ...
    const nuevas = unidades.filter(u => !ids.has(u.id));
    if (nuevas.length) {
    ...
    await conn.commit();
    res.redirect(`/aresep?periodo=${req.periodoAresep}&seccion=unidades&incorporados=1`);
...
router.get('/exportar/:tipo', contexto, envolver(async (req, res) => {
  ...
  const buffer = await generarExcel(req.params.tipo, req.periodoAresep, rows, a.permitidas(req.session.user).includes('planilla'));
  res.type('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  res.attachment(`ARESEP_${req.params.tipo}_${req.periodoAresep}.xlsx`);
